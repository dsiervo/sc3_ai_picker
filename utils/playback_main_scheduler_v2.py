            ...
            if event_id:
                ...
                if eval_mode == 'manual' or pref_status == 'reported':
                    ...
                    if author == 'EQCCT':
                        ...
                        if pref_mag is not None and float(pref_mag) < 1.9:
...
def runner(starttime: str, endtime: str, ai_scheduler_file: str, every_m: int = 40, dispatch: bool = False, overlap: float = 0.3):
    ...
    while t_i < t:
        ...
        ai_picker_inp = change_times(t_i.strftime("%Y-%m-%d %H:%M:%S"),
                     tf,
                     delta, ai_scheduler_file)

        print_ic(f'\nrunning from {t_i} to {tf}\n')
        ...
        # Increment the start time by every_m minutes
        t_i += datetime.timedelta(minutes=every_m)
